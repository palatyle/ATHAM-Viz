#!/bin/bash
# Sets up and submits a batch of ATHAM runs to the PBS queue for small vent sizes (smaller grid spacing)
...
i_flag="-i IO_ref "
f_flag=" -f atham "
a_flag="-a INPUT_atham_setup_small_grid_10m_vent "
d_flag=" -d INPUT_dynamic_setup_high_iter"
        ...
        for velocity in ${velocities[@]}; do
            for windspeed in ${windspeeds[@]}; do
                ...
                if [ "${lat}" = "tropical" ]; then
                    p_flag='-p INPUT_profile_'${windspeed}mps'_trop '
