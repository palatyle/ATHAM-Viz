    ...
    for ventsize in ${ventsizes[@]}; do
        for velocity in ${velocities[@]}; do
            for windspeed in ${windspeeds[@]}; do
                ...
                pbs_name=${lat}_flat${ventsize}m_${velocity}ms_${windspeed}ms.batch

                # Copy master pbs file
                cp ../PBS_files/SLURM_template.batch $pbs_name

                # Edit -N flag in new pbs file
                N_var='5s/.*/''#SBATCH --job-name='${lat}_flat${ventsize}m_${velocity}ms_${windspeed}ms'/'
                sed -i "$N_var" $pbs_name

                # Create string to edit run line

                # set output directory and create it 
                out_dir='/lfs/palatyle.isu/output/'${lat}_flat${ventsize}m_${velocity}ms_${windspeed}ms
                mkdir $out_dir
                o_flag='-o '$out_dir
